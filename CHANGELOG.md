...
## [Unreleased]
### Added
- Added an optional `period` argument to `repeat_units()` to return consensus repeat units for a specified period present in the decoded Viterbi path.

### Changed
- Bump `pyo3` to `0.29` and `thiserror` to `2.0.19`.
- Masker subclass initialization was migrated to `PyClassInitializer`, replacing deprecated tuple-based superclass initialization.
